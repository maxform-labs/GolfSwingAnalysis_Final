...
### 📐 표준 캘리브레이션 파일 사용 가이드 (Phase 2 완료)

**⚠️ 중요**: 모든 골프 스윙 분석에는 표준 캘리브레이션 파일을 사용하세요.
...
#### 주요 특징
- **베이스라인**: 470.0mm (2024-10-29 통일 완료)
- **Y축 스케일링**: 0.2778 (1440x300 해상도 최적화)
- **초점거리**: 400.0px (스케일링 적용됨)
...
scale_factor_y = calibration_data['scale_factor_y']  # 0.2778

# 깊이 계산 (수직 스테레오 비전)
depth = (baseline * focal_length) / disparity_y  # mm
```

#### 캘리브레이션 파일 선택 가이드

| 파일명 | 용도 | 베이스라인 | Y축 스케일링 | 권장 |
|--------|------|-----------|-------------|------|
| `config/calibration_default.json` | **표준 (권장)** | 470.0mm | ✅ 적용됨 | ⭐⭐⭐ |
| `improved_calibration_470mm.json` | 대안 | 470.0mm | ✅ 적용됨 | ⭐⭐ |
| `manual_calibration_470mm.json` | 원본 (비권장) | 470.0mm | ❌ 미적용 | ⭐ |

#### 베이스라인 470mm 통일 작업 (2024-10-29 완료)

**Phase 1 완료사항**:
- ✅ 레거시 베이스라인 값(236.76mm, 289.35mm) 제거
- ✅ Python 파일 2개 수정 (`final_trajectory_tracker.py`, `fixed_3d_tracker.py`)
- ✅ 표준 캘리브레이션 파일 생성 (`config/calibration_default.json`)
- ✅ 변경사항 검증 완료 (27개 코드 라인에서 470mm 사용 확인)

**Phase 2 완료사항**:
- ✅ 캘리브레이션 파라미터 검증 (`verify_calibration_parameters.py`)
- ✅ 회귀 테스트 통과 (`verify_baseline_changes.py`)
- ✅ 문서 업데이트 (이 가이드 추가)

**측정 정확도 개선**:
- 1m 거리: ±6.4mm (기존 ±10.4mm 대비 38% 개선)
- 2m 거리: ±25.5mm (기존 ±41.5mm 대비 38% 개선)
- 3m 거리: ±57.4mm (기존 ±93.4mm 대비 38% 개선)

**관련 문서**:
- Phase 1 보고서: `docs/baseline_unification_report.md`
- Phase 2 계획: `docs/after-calibration-plan.md`
- 검증 스크립트: `verify_calibration_parameters.py`, `verify_baseline_changes.py`

#!/usr/bin/env python3
"""
캘리브레이션 이미지를 사용한 골프공 물리량 분석
전체 해상도 이미지로 정확한 스테레오 매칭
"""
...
class CalibrationPhysicsAnalyzer:
    def __init__(self, calibration_file="vertical_stereo_calibration_z_axis.json"):
        ...
        print("Calibration Physics Analyzer Initialized")
        ...
        print("Using full resolution calibration images")
    ...
    def detect_golf_ball_adaptive(self, frame: np.ndarray) -> tuple:
        ...
        for contour in contours:
            ...
            if 50 < area < 10000:  # 전체 해상도에 맞는 면적 조건
    ...
    def analyze_calibration_sequence(self, calibration_dir, output_dir):
        """캘리브레이션 시퀀스 분석"""
        print(f"\n=== ANALYZING CALIBRATION SEQUENCE ===")
        ...
        if len(positions_3d) >= 2:
            ...
            if velocity is not None:
                ...
                # 결과 저장
                result = {
                    'camera_system': 'Calibration_Stereo',
                    'fps': self.fps,
                ...
                # JSON 파일로 저장
                result_file = os.path.join(output_dir, "calibration_physics_analysis.json")
...
def main():
    """메인 함수"""
    analyzer = CalibrationPhysicsAnalyzer()
    
    calibration_dir = "data2/Calibration_image_1025"
    output_dir = "calibration_physics_result"
    
    if os.path.exists(calibration_dir):
        analyzer.analyze_calibration_sequence(calibration_dir, output_dir)
    else:
        print(f"Calibration directory {calibration_dir} not found")
    
    print("\n=== CALIBRATION ANALYSIS COMPLETE ===")
    print("Check the calibration_physics_result directory for detailed results!")

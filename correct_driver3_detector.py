...
class CorrectDriver3Detector:
    def __init__(self, input_dir="data2/driver/3", output_dir="driver_3_correct_result"):
        """정확한 Driver 3 골프공 검출기 초기화"""
        self.input_dir = input_dir
        self.output_dir = output_dir
        os.makedirs(self.output_dir, exist_ok=True)
        
        print("Correct Driver 3 Golf Ball Detector Initialized")
        print(f"Input directory: {self.input_dir}")
        print(f"Output directory: {self.output_dir}")
        
        # 실제 골프공이 있는 프레임 정의
        self.golf_ball_frames = {
            'Cam1': list(range(1, 14)),  # 1~13
            'Cam2': list(range(1, 10))   # 1~9
        }
    ...
    def process_all_cameras(self):
        """모든 카메라 처리"""
        print("=== CORRECT DRIVER 3 GOLF BALL DETECTION SYSTEM ===")
        print("Processing Cam1 and Cam2 sequences with correct frame mapping")
...
def main():
    """메인 함수"""
    detector = CorrectDriver3Detector()
    
    # 모든 카메라 처리
    results, stats = detector.process_all_cameras()
    
    print(f"\nCorrect Driver 3 golf ball detection completed!")

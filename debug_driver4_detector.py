#!/usr/bin/env python3
"""
디버그용 Driver 4 골프공 검출 시스템
이미지를 직접 확인하고 적절한 검출 조건 찾기
"""
...
class DebugDriver4Detector:
    def __init__(self, input_dir="data2/driver/4", output_dir="driver_4_debug_result"):
        """디버그용 Driver 4 골프공 검출기 초기화"""
        self.input_dir = input_dir
        self.output_dir = output_dir
        os.makedirs(self.output_dir, exist_ok=True)
        
        print("Debug Driver 4 Golf Ball Detector Initialized")
        print(f"Input directory: {self.input_dir}")
        print(f"Output directory: {self.output_dir}")
        print("Debugging detection parameters")
    
    def analyze_image_properties(self, img, frame_num, camera_name):
        """이미지 속성 분석"""
        ...
        print(f"  {camera_name} Frame {frame_num}: Brightness - Mean: {mean_brightness:.1f}, Max: {max_brightness}, Min: {min_brightness}")
        
        # 밝은 영역 분석
        bright_pixels = gray[gray > 200]
        very_bright_pixels = gray[gray > 220]
        
        print(f"    Pixels > 200: {len(bright_pixels)} ({len(bright_pixels)/gray.size*100:.1f}%)")
        print(f"    Pixels > 220: {len(very_bright_pixels)} ({len(very_bright_pixels)/gray.size*100:.1f}%)")
        
        return mean_brightness, max_brightness, min_brightness
    ...
    def detect_golf_ball_adaptive(self, frame: np.ndarray, frame_num: int, camera_name: str) -> tuple:
        ...
        print(f"    Adaptive thresholds: {threshold_low}-{threshold_high}")
                ...
                if perimeter > 0:
                    ...
                    if circularity > 0.5:
                        ...
                        if mean_brightness_roi > threshold_low * 0.8:
                            ...
                            print(f"    Contour {i}: Area={area:.0f}, Circularity={circularity:.2f}, Brightness={mean_brightness_roi:.1f}, Score={score:.2f}")
        ...
        if circles is not None:
            ...
            for circle in circles:
                ...
                if mean_brightness > threshold_low * 0.8:
                    print(f"    Hough detection: Center=({center[0]}, {center[1]}), Radius={radius}, Brightness={mean_brightness:.1f}")
                    return center, radius, mask
        
        print(f"    No golf ball detected")
    ...
    def process_camera_sequence(self, camera_name, max_frames=10):
        """카메라 시퀀스 처리 (처음 몇 프레임만)"""
        print(f"\n=== PROCESSING {camera_name} SEQUENCE (First {max_frames} frames) ===")
        ...
        for i, file_path in enumerate(files[:max_frames]):
            ...
            print(f"\n{camera_name} Frame {frame_num:2d}:")
            ...
            # 이미지 속성 분석
            mean_brightness, max_brightness, min_brightness = self.analyze_image_properties(img, frame_num, camera_name)
    ...
    def process_all_cameras(self):
        """모든 카메라 처리 (디버그용)"""
        print("=== DEBUG DRIVER 4 GOLF BALL DETECTION SYSTEM ===")
        print("Analyzing first 10 frames of each camera for debugging")
        
        all_results = {}
        all_stats = {}
        
        # Cam1 처리 (처음 10프레임만)
        cam1_results, cam1_stats = self.process_camera_sequence('Cam1', max_frames=10)
        all_results['Cam1'] = cam1_results
        all_stats['Cam1'] = cam1_stats
        
        # Cam2 처리 (처음 10프레임만)
        cam2_results, cam2_stats = self.process_camera_sequence('Cam2', max_frames=10)
...
def main():
    """메인 함수"""
    detector = DebugDriver4Detector()
    
    # 모든 카메라 처리 (디버그용)
    results, stats = detector.process_all_cameras()
    
    print(f"\nDebug Driver 4 golf ball detection completed!")
    print(f"Check the debug images to analyze detection parameters!")

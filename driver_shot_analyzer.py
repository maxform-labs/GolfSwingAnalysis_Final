...
"""
드라이버 샷 분석기
최적 좌표계를 사용하여 20개 드라이버 샷의 물리량 분석
"""
...
class DriverShotAnalyzer:
    def __init__(self, calibration_file="vertical_stereo_calibration_z_axis.json"):
        ...
        self.optimal_coordinate_system = {
            'name': 'Optimal (X disparity for driver images)',
        ...
        print("Driver Shot Analyzer Initialized")
        print(f"Using optimal coordinate system: {self.optimal_coordinate_system['name']}")
    ...
    def calculate_physics_with_optimal_coordinates(self, positions_3d, frame_numbers):
        """최적 좌표계에서 물리량 계산"""
    ...
    def analyze_single_shot(self, shot_dir, shot_number):
        """단일 샷 분석"""
        print(f"\n--- Analyzing Shot {shot_number} ---")
        ...
        if len(positions_3d) >= 2:
            print(f"  Detected {len(positions_3d)} 3D positions")
            
            # 물리량 계산
            physics = self.calculate_physics_with_optimal_coordinates(positions_3d, frame_numbers)
    ...
    def analyze_all_shots(self, driver_dir="data2/driver", output_dir="driver_analysis_result"):
        """모든 드라이버 샷 분석"""
        print(f"\n=== ANALYZING ALL DRIVER SHOTS ===")
        ...
        for shot_dir in data_dirs:
            ...
            result = self.analyze_single_shot(shot_path, shot_number)
            results.append(result)
        
        # 결과 저장
        self.save_results(results, real_data, output_dir)
        
        # 비교 리포트 생성
        self.generate_comparison_report(results, real_data, output_dir)
    ...
    def save_results(self, results, real_data, output_dir):
        """결과 저장"""
        # JSON 결과 저장
        json_results = []
        ...
        with open(os.path.join(output_dir, "driver_shot_analysis.json"), 'w', encoding='utf-8') as f:
        ...
        df_results.to_csv(os.path.join(output_dir, "driver_shot_analysis.csv"), index=False)
        
        print(f"\nResults saved to {output_dir}/")
        print(f"  - driver_shot_analysis.json")
        print(f"  - driver_shot_analysis.csv")
    
    def generate_comparison_report(self, results, real_data, output_dir):
        """비교 리포트 생성"""
        ...
        if comparison_data:
            ...
            print(f"\nComparison data saved to: {output_dir}/comparison_with_real_data.csv")

def main():
    """메인 함수"""
    analyzer = DriverShotAnalyzer()
    
    driver_dir = "data2/driver"
    output_dir = "driver_analysis_result"
    
    if os.path.exists(driver_dir):
        results = analyzer.analyze_all_shots(driver_dir, output_dir)

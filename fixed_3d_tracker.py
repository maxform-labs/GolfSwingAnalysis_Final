...
class Fixed3DTracker:
    def __init__(self):
        """수정된 3D 궤적 추적기 초기화"""
        self.output_base_path = "fixed_3d_results"
        
        # 궤적 추적을 위한 파라미터
        self.max_displacement = 100  # 프레임 간 최대 이동 거리 (픽셀)
    ...
    def detect_ball_working(self, img):
        """작동하는 볼 검출"""
        try:
            # 그레이스케일 변환
    ...
    def detect_ball_in_frame(self, img, previous_ball=None):
        """단일 프레임에서 볼 검출"""
        try:
            ball = self.detect_ball_working(img)
    ...
    def calculate_3d_position_fixed(self, x1, y1, x2, y2):
        """수정된 3D 위치 계산 (X축 disparity 사용)"""
        if not self.calibration_data:
            return None, None, None
        
        try:
            # X축 disparity 계산 (수평 스테레오)
            ...
            # 3D 좌표 계산
            if disparity_x > 0 and disparity_x < 200:  # 합리적인 disparity 범위
                z_3d = (baseline * focal_length) / disparity_x
                
                # 합리적인 거리 범위 체크 (0.1m ~ 10m)
                if 100 <= z_3d <= 10000:
    ...
    def calculate_3d_trajectory(self, trajectory1, trajectory2):
        ...
        for i in range(min_length):
            try:
                ...
                # 3D 좌표 계산
                x_3d, y_3d, z_3d = self.calculate_3d_position_fixed(
    ...
    def calculate_ball_speed_fixed(self, trajectory_3d):
        """수정된 볼 속도 계산"""
    ...
    def process_single_shot(self, shot_path, club_name, shot_num):
        ...
        # 속도 계산
        speed_info = self.calculate_ball_speed_fixed(trajectory_3d)
    ...
    def process_all_shots(self, data_path):
        ...
        for club_folder, club_name in clubs.items():
            ...
            logger.info(f"Processing {club_name} shots for fixed 3D tracking...")
            
            # 샷 디렉토리 찾기
            shot_dirs = [d for d in os.listdir(club_path) if d.isdigit()]
            shot_dirs.sort(key=int)
            
            for shot_num in shot_dirs[:3]:  # 처음 3개 샷만 처리
...
def main():
    """메인 실행 함수"""
    logger.info("Starting fixed 3D ball trajectory tracking...")
    
    tracker = Fixed3DTracker()
    ...
    if os.path.exists(data_path):
        ...
        logger.info(f"Fixed 3D trajectory tracking completed! Processed {len(results)} shots")

...
"""
골프공 물리량 분석 시스템 (수정版)
ROI와 캘리브레이션을 바탕으로 속도, 발사각, 방향각 계산
"""
...
class GolfBallPhysicsAnalyzer:
    ...
    def analyze_driver_sequence(self, driver_dir, output_dir):
        ...
        # ROI 로드
        roi_cam1 = self.load_roi(os.path.join(driver_dir, "roi_cam1.json"))
        roi_cam2 = self.load_roi(os.path.join(driver_dir, "roi_cam2.json"))
        ...
        for camera in ['Cam1', 'Cam2']:
            ...
            for file_path in files:
                ...
                # ROI 적용
                roi_data = roi_cam1 if camera == 'Cam1' else roi_cam2
                x_offset = roi_data['XOffset']
                y_offset = roi_data['YOffset']
                width = roi_data['Width']
                height = roi_data['Height']
                
                # ROI 범위 확인
                if (y_offset + height > img.shape[0] or 
                    x_offset + width > img.shape[1] or
                    y_offset < 0 or x_offset < 0):
                    print(f"Frame {frame_num}: ROI out of bounds")
                    continue
                
                roi_img = img[y_offset:y_offset+height, x_offset:x_offset+width]
                
                if roi_img.size == 0:
                    print(f"Frame {frame_num}: Empty ROI")
                    continue
                
                print(f"Frame {frame_num}: ROI size {roi_img.shape}")
                
                # 골프공 검출
                center, radius = self.detect_golf_ball_adaptive(roi_img)
                
                if center is not None:
                    # 전체 이미지 좌표로 변환
                    global_center = (center[0] + x_offset, center[1] + y_offset)
                    positions.append(global_center)
                    timestamps.append(frame_num * 0.033)  # 30fps 가정
                    frame_numbers.append(frame_num)
                    
                    print(f"Frame {frame_num}: Detected at {global_center}")
                else:
                ...
                if len(positions_3d) >= 2:
                    ...
                    if velocity is not None:
                        ...
                        result = {
                            ...
                            'positions_3d': [pos.tolist() for pos in positions_3d],

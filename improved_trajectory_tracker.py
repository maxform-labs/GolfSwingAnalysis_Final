...
class ImprovedTrajectoryTracker:
    def __init__(self):
        """개선된 볼 궤적 추적기 초기화"""
        self.output_base_path = "ball_trajectory_results"
        
        # 궤적 추적을 위한 파라미터
        self.max_displacement = 200  # 프레임 간 최대 이동 거리 (픽셀)
    ...
    def detect_ball_enhanced(self, img):
        """향상된 볼 검출"""
        try:
            # 그레이스케일 변환
            ...
            if circles3 is not None:
                circles_list.extend(circles3[0])
            ...
            if circles_list:
                ...
                if len(circles_array) > 0:
                    # 반지름 기준으로 정렬
                    circles_array = circles_array[circles_array[:, 2].argsort()[::-1]]
                    x, y, r = circles_array[0]
                    return (int(x), int(y), int(r))
            
            return None
            
        except Exception as e:
            logger.warning(f"Enhanced ball detection error: {str(e)}")
            return None
    ...
    def detect_ball_in_frame(self, img, previous_ball=None):
        ...
        try:
            # 향상된 검출
            ball = self.detect_ball_enhanced(img)
...
def main():
    """메인 실행 함수"""
    logger.info("Starting improved ball trajectory tracking...")
    
    tracker = ImprovedTrajectoryTracker()

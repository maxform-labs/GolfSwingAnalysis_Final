...
def test_new_calibration():
    ...
    for i in range(min(5, len(files_cam1), len(files_cam2))):
        ...
        if center1 is not None and center2 is not None:
            ...
            # 깊이 계산 시뮬레이션 (새로운 캘리브레이션)
            focal_length = 1500  # 새로운 캘리브레이션의 초점거리
            baseline = 470
            
            # X 시차로 깊이 계산
            x_disparity = u1 - u2
            if x_disparity != 0:
                x_depth = (focal_length * baseline) / abs(x_disparity)
